...
public class TwitterSnscrape {
    ...
    public static void init() {
        ...
        generatorEntries = initializeOutputWriting("/home/abdalla/Desktop/entries.json");
        generator = initializeOutputWriting("/home/abdalla/Desktop/SNS.json");
    }
    ...
    private static List<String> _iter_api_data(String endpoint, Map params, Map paginationParams, String cursor) {
        ...
        while(true) {
            ...
            // TEST: write json response to check the final results
            try {
                mapper.writeValue(generatorEntries, jsonResponse);
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    private static JsonGenerator initializeOutputWriting(String outputPath) {
...
// --->obj: _iter_api_data() - 'https://api.twitter.com/2/search/adaptive.json', params, paginationParams
// ------> if cursor == none -> take reqParams = params else take reqParams = pagingParams and update cursor
// ------> while true
// ---------> obj = self._get_api_data(endpoint, reqParams)
// ------------> _ensure_guest_token()
// ---------------> if _guestToken is not None: return
// ---------------> _get(self._baseUrl if url is None else url, headers = {'User-Agent': self._userAgent})
// ---------------> use regex _guestToken = match.group(1)
// ---------------> if 'gt' in r.cookies: Found guest token in cookies self._guestToken = r.cookies['gt']
// ---------------> if self._guestToken:
...
// ------------> r = _get(endpoint, params = params, headers = self._apiHeaders, responseOkCallback = self._check_api_response)
// ------------> obj = r.json() return obj
// ---------> yield obj
// ---------> update cursor and reqParams
// --->_instructions_to_tweets(obj)
// -> print each json item - json.dumps(namedtuple_to_dict_recursive(item), default = json_serialise_datetime))